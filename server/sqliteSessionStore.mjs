...
export function createSqliteSessionStore({ filename, now = Date.now }) {
  if (!filename || filename === ":memory:")
    throw new Error("durable_session_path_required");
  const db = new DatabaseSync(filename);
  try {
    db.exec(
      "PRAGMA journal_mode=WAL; PRAGMA synchronous=FULL; PRAGMA busy_timeout=5000; CREATE TABLE IF NOT EXISTS sessions (id TEXT PRIMARY KEY, value TEXT NOT NULL, expires INTEGER NOT NULL)",
    );
  } catch {
    db.close();
  ...
  return {
    ...
    get(id) {
      const row = db
        .prepare("SELECT value, expires FROM sessions WHERE id=?")
        .get(id);
      if (!row) return null;
      if (row.expires <= now()) {
        this.delete(id);
