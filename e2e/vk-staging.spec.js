import { chooseVkMetadata } from "./vkJourney.helpers.js";
...
test("VK entry clears launch query, saves metadata and restores server wardrobe", async ({
  ...
  await expect(
    page.getByRole("heading", { name: /Немного о вашем стиле/ }),
  ).toBeVisible();
  await chooseVkMetadata(page);
  await expect(
    page.getByRole("button", { name: "Сохранить без фото" }),
  ).toBeEnabled();
