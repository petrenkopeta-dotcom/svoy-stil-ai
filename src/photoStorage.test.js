...
test("photo storage requires explicit versioned consent", async () => {
  await assert.rejects(createPhotoStorage({ validateCutout: async () => true, backend: new MemoryPhotoBackend() }).save(image(), { granted: true }), (error) => error.code === PHOTO_STORAGE_CODES.CONSENT_REQUIRED);
});

test("photo storage stores Blob and deletes expired data", async () => {
  let time = 100;
  const store = createPhotoStorage({ validateCutout: async () => true, backend: new MemoryPhotoBackend(), now: () => time, randomId: () => "p1", retentionMs: 10 });
...
test("memory fallback is explicitly non-persistent", async () => {
  const store = createPhotoStorage({ validateCutout: async () => true, indexedDB: null, randomId: () => "fallback" });
...
test("quota failures have a stable code", async () => {
  ...
  await assert.rejects(createPhotoStorage({ validateCutout: async () => true, backend }).save(image(), consent), (error) => error.code === PHOTO_STORAGE_CODES.QUOTA_EXCEEDED);
});

test("legacy migration is copy-first and rollback-safe", async () => {
  const store = createPhotoStorage({ validateCutout: async () => true, backend: new MemoryPhotoBackend(), randomId: () => "migrated" });
...
test("deleteAll clears photo bytes", async () => {
  const store = createPhotoStorage({ validateCutout: async () => true, backend: new MemoryPhotoBackend(), randomId: () => "p1" });
  await store.save(image(), consent); await store.deleteAll();
  await assert.rejects(store.get("p1"), (error) => error.code === PHOTO_STORAGE_CODES.NOT_FOUND);
});

test("delete returns a verifiable local receipt", async () => {
  const store = createPhotoStorage({ validateCutout: async () => true, backend: new MemoryPhotoBackend(), randomId: () => "receipt", now: () => 77 });
...
test("deleteExpired removes all due records", async () => {
  let time = 1; let id = 0;
  const store = createPhotoStorage({ validateCutout: async () => true, backend: new MemoryPhotoBackend(), now: () => time, retentionMs: 5, randomId: () => `p${++id}` });
